...
def create_version_folders():
    """10000개 단위로 버전 폴더 생성"""
    base_dir = "saved_models/multilayer/hidden4"
...
def backup_hidden4_files():
    """은닉층 4개 다층 퍼셉트론 파일들을 백업 폴더로 이동"""
    backup_dir = "saved_models/multilayer/hidden4"
    ...
    for file in files_to_backup:
        if os.path.exists(file):
            ...
            print(f"은닉층 4개 다층 퍼셉트론 백업 완료: {file} → {backup_dir}")
...
def main():
    ...
    for episode in range(config['num_episodes']):
        ...
        if (episode+1) % 100 == 0:
            print(f"은닉층 4개 다층 퍼셉트론 - 에피소드 {episode+1}, 총 보상: {total_reward:.2f}, epsilon: {epsilon:.3f}, decay: {epsilon_decay:.4f}")
        # 1만 에피소드마다 모델 저장 및 진행상황 출력
        if (episode+1) % 10000 == 0:
            version = (episode+1) // 10000
            model_path = f"hidden4_perceptron_v{version}.pth"
            torch.save(agent.state_dict(), model_path)
            print(f"[진행상황] 은닉층 4개 다층 퍼셉트론 {episode+1} 에피소드 완료, 모델 저장: {model_path}")
    ...
    print("은닉층 4개 다층 퍼셉트론 모델 및 성능지표 저장 완료!")

    # 파일 백업
    backup_hidden4_files()
    
    print("\n=== 은닉층 4개 다층 퍼셉트론 학습 완료! ===")
    print("결과: saved_models/multilayer/hidden4/")
